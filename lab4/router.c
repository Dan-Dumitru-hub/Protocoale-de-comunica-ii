...
//memcpy(eth_hdr->ether_dhost,get_best_route(ip_hdr->saddr),6);
...
//memcpy(eth_hdr->ether_dhost,get_arp_entry(ip_hdr->daddr)->mac,6);



//printf("trimit catre %s\n",m.interface);

		struct arp_entry * arpe=get_arp_entry(ip_hdr->daddr);
...
//memcpy(eth_hdr->ether_shost,eth_hdr->ether_dhost,6);
memcpy(eth_hdr->ether_dhost,arpe->mac,sizeof(arpe->mac));
